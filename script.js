...
function zhangFormula(weight, volume, packing) {
  ...
  let unitPrice = packing === "wood" ? 130 : 105;
  let packingCost = realCub * unitPrice;
...
// // service-card click -> modal ochiladi
// document.querySelectorAll(".service-card").forEach(card => {
//   card.addEventListener("click", () => {
//     const key = card.dataset.service;
//     const data = serviceData[key][currentLang];
...
//   // sahifadagi barcha data-uz / data-en yangilanadi
...
// // misol: til tugmalari
// document.getElementById("btnUz").addEventListener("click", () => switchLang("uz"));
...
document.getElementById("orderForm").addEventListener("submit", function(e) {
    ...
    async function translatePage(lang) {
      let elements = document.querySelectorAll("#title, #desc");

      for (let el of elements) {
        let original = el.getAttribute("data-original") || el.innerText;
        el.setAttribute("data-original", original); // asl matnni saqlash

        let translated = await translateText(original, lang);
